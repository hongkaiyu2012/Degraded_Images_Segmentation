#!/usr/bin/env python
# custom data loader follow CamVid's data format
...
class CustomSeg(data.Dataset):
    ...
    def __init__(self, root, split='train', dataset='o', transform=False):
        ...
        for split in ['train', 'val']:
            ...
            for did in open(imgsets_file):
                did = did.strip()
                img_file = osp.join(img_dataset_dir, 'Custom_train_images/%s.png' % did)
                lbl_file = osp.join(root, 'Custom_train_gt/%s.png' % did)
        ...
        for did in open(imgsets_file):
            did = did.strip()
            img_file = osp.join(img_dataset_dir, 'Custom_test_images/%s.png' % did)
            lbl_file = osp.join(root, 'Custom_test_gt/%s.png' % did)
    ...
    def __getitem__(self, index):
        ...
        # yu55 debug 04/26/2019: not care loss during testing, use zeros as gt label
        lbl = np.zeros([360,480])
